...
async def seed_data():
    async with AsyncSessionLocal() as db:
        try:
            ...
            users = []
            try:
                admin = await user_service.create(email="admin@fantasy.com", username="admin", password="adminpassword123", role="admin")
                users.append(admin)
            except Exception:
                user = await user_service.get_by_email("admin@fantasy.com")
                if user: users.append(user)

            for i in range(1, 6):
                try:
                    u = await user_service.create(email=f"user{i}@test.com", username=f"user_{i}", password="testpassword123")
                    users.append(u)
                except Exception:
                    user = await user_service.get_by_email(f"user{i}@test.com")
                    if user: users.append(user)
            ...
            for t in team_data:
                try:
                    team = await team_service.create(name=t["name"], region=t["region"], logo_url=t["logo_url"])
                    teams.append(team)
                except Exception:
                    team = await team_service.repo.get_by_name(t["name"])
            ...
            try:
                league = await league_service.create(name="Pro Valorant League", admin_user_id=users[0].id)
                
                # Join users to league
                for user in users:
                    try:
                        member = await member_service.join_league(
                            league_id=league.id,
                            user_id=user.id,
                        ...
                        all_players = await player_service.get_all()
                        sample_players = random.sample(all_players, min(len(all_players), 5))
                        for p in sample_players:
                            try:
                                await roster_service.add_player(
            ...
            for i in range(5):
                try:
                    ...
                    t1_players = await player_service.get_by_team(t1.id)
                    t2_players = await player_service.get_by_team(t2.id)
                    
                    for p in t1_players + t2_players:
                        try:
                            await stats_service.create(
                                match_id=match.id,
                                player_id=p.id,
                                agent="Jett",
